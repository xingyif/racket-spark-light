...
A DataShell is an immutable structure that stores user input Data, on which DSOperation are performed. The user is able to create a DataShell by:
...
All Transformations are lazy, which means that they are not evaluated when the user calls them. Rather, RSL records a graph of transformations applied on the base DataShell (provided by the user).
The Transformations are only analyzed, optimized, and computed when an Action is applied on the same DataShell. Using lazy Transformations allow RSL to evaluate and optimize the operations applied on the DataShell, and traverse the list as few times as possible. Therefore, it improves the performance of processing large data sets.
...
The user is able to use any of the supported DSOperations on DataShells, however, the user is not allowed to determine whether an operation is a Transformation or an Action.


A Lazy Evaluation is a strategy that purposefully delays the evaluation of the given function until the value is needed. RSL intelligently uses laziness to improve the performance of processing large data sets.

An Eager Evaluation is a strategy that evaluates the given function as soon as it is provided. It is the opposite of a Lazy Evaluation. In RSL, Eager Evaluations trigger Lazy Evaluations.
